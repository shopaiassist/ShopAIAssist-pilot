    ...
    it('should return index page', () => {
      return chai
        ...
        .then((res) => {
          ...
          expect(sendFileSpy.getCall(0).args[0]).to.contain('client');
          expect(sendFileSpy.getCall(0).args[0]).to.contain('dist');
          expect(sendFileSpy.getCall(0).args[0]).to.contain('index.html');
        });
    });
  });
});
